...
from modules.winrm.encryption import Encryption
from modules.winrm.exceptions import InvalidCredentialsError, WinRMError, WinRMTransportError

DISPLAYED_PROXY_WARNING = False
DISPLAYED_CA_TRUST_WARNING = False

from modules.winrm.vendor.requests_pku2u import HttpPku2uAuth
...
class Transport(object):
    def __init__(
        ...
        auth_method: t.Literal["auto", "basic", "certificate", "ntlm", "kerberos", "credssp", "plaintext", "ssl", "pku2u"] = "auto",
        ...
        if self.auth_method != "kerberos":
            ...
            elif self.auth_method == "pku2u":
                if not self.pfx or not self.pfxpass:
                    raise InvalidCredentialsError("p2p cert should be specified")
            else:
    ...
    def build_session(self) -> requests.Session:
        ...
        # TODO: ssl is not exactly right here- should really be client_cert
            ...
            session.auth = HttpPku2uAuth(self.endpoint, self.pfx, self.pfxpass)
        else:
    ...
    def send_message(self, message: str | bytes) -> bytes:
        ...
        if self.auth_method == "pku2u" and session.auth.auth_done == False:
            while session.auth.interaction < 4:
                ...
                request = requests.Request("POST", self.endpoint)
                prepared_request = session.prepare_request(request)
