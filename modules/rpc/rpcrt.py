...
class DCERPC_v5(DCERPC):
    def __init__(self, transport):
        ...
        self.__TGS    = None
        self.__p2ppfx   = None
        self.__p2ppfxpass = None
        self.__p2phelper = None
    ...
    def set_credentials(self, username, password, domain = '', lmhash = '', nthash = '', aesKey = '', TGT = None, TGS = None, p2ppfx='', p2ppfxpass=''):
        ...
        self.__TGT      = TGT
        self.__TGS      = TGS
        self.__p2ppfx   = p2ppfx
        self.__p2ppfxpass = p2ppfxpass

        if self.__p2phelper != '' and self.__p2ppfxpass != '':
            self.__p2phelper = NegoExHelper(self.__p2ppfx, self.__p2ppfxpass, self._transport.getRemoteName())
        if lmhash != '' or nthash != '':
            if len(lmhash) % 2:
                lmhash = '0%s' % lmhash
            if len(nthash) % 2:
    ...
    def bind(self, iface_uuid, alter = 0, bogus_binds = 0, transfer_syntax = ('8a885d04-1ceb-11c9-9fe8-08002b104860', '2.0')):
        ...
        if self.__auth_level != RPC_C_AUTHN_LEVEL_NONE:
            ...
            if self.__auth_type == RPC_C_AUTHN_WINNT:
                ...
                if self.__p2ppfx and self.__p2ppfxpass:
                    ...
                    mechToken = self.__p2phelper.GenerateNegoExInit()
        ...
        if self.__auth_level != RPC_C_AUTHN_LEVEL_NONE:
            if self.__auth_type == RPC_C_AUTHN_WINNT:
                ...
                if self.__p2phelper:
                    mechToken = self.__p2phelper.GenerateNegoExKerberosAs(bindResp['auth_data'])
                    blob = SPNEGO_NegTokenResp()
                    blob['ResponseToken'] = bytes.fromhex(mechToken)
                    response = blob.getData()
                else:
            ...
            if response is not None:
                if self.__auth_type == RPC_C_AUTHN_GSS_NEGOTIATE:
                    ...
                    if self.__p2phelper:
                        ...
                        resp = MSRPCHeader(response_data)
                        bindResp = MSRPCBindAck(resp.getData())
                        try:
                            mechToken = self.__p2phelper.GenerateNegoExKerberosAp(bindResp['auth_data'])
                        ...
                        msgs = negoexts_parse_bytes(bindResp['auth_data'][21:])
                        ...
                        subkey_key = Key(cipher.enctype, self.__p2phelper.session_key.contents)
                        temp = cipher.decrypt(subkey_key, 12, cipher_text)
                        enc_part = EncAPRepPart.load(temp).native
                        self.__cipher  = _enctype_table[int(enc_part['subkey']['keytype'])]()
                        self.__sessionKey = Key(self.__cipher .enctype, enc_part['subkey']['keyvalue'])
                        self.__gss = gssapi.GSSAPI(cipher)
                        from asyauth.protocols.kerberos.gssapi import get_gssapi
                        self.__gss2 = get_gssapi(self.__sessionKey)
                ...
                if sec_trailer['auth_level'] == RPC_C_AUTHN_LEVEL_PKT_PRIVACY:
                    if self.__auth_type == RPC_C_AUTHN_WINNT:
                        ...
                        if self.__sequence > 0:
                            krb5_header = auth_data[8:-16]
                            answer, cfounder = self.__gss2.GSS_Unwrap(krb5_header+answer, 0, direction='init', auth_data=auth_data)
