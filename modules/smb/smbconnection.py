...
from impacket import nmb, nt_errors, LOG
...
from impacket.smb3structs import SMB2Packet, SMB2_DIALECT_002, SMB2_DIALECT_21, SMB2_DIALECT_30, GENERIC_ALL, FILE_SHARE_READ, \
    ...
    FILE_OPEN_REPARSE_POINT, FSCTL_SET_REPARSE_POINT, SMB2_0_IOCTL_IS_FSCTL, \
...
from modules.smb.smb3 import SMB3
...
class SMBConnection:
    ...
    def negotiateSession(self, preferredDialect=None,
        ...
        if preferredDialect is None:
            ...
            if packet[0:1] == b'\xfe':
                ...
                self._SMBConnection = SMB3(self._remoteName, self._remoteHost, self._myName, hostType,
    ...
    def login(self, user, password, domain = '', lmhash = '', nthash = '', ntlmFallback = True):
        ...
        except (smb.SessionError, SessionError) as e:
    ...
    def kerberosLogin(self, user, password, domain='', lmhash='', nthash='', aesKey='', kdcHost=None, TGT=None,
        ...
        :param struct TGS: same for TGS. See py for the format
    ...
    def kerberosCertificateLogin(self, userCert, certPass):
        if self.getDialect() == smb.SMB_DIALECT:
            raise Exception("SMB1 not implemented yet")

        return self._SMBConnection.kerberosCertificateLogin(userCert, certPass)
