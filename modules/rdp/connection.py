...
class RDPConnection:
	...
	async def connect(self):
		...
		try:
			...
			if self.client_x224_supported_protocols is None and self.credentials is not None:
				...
				elif self.credentials.protocol is asyauthProtocol.SPNEGOEX:
					self.client_x224_flags = 0
					self.client_x224_supported_protocols = SUPP_PROTOCOLS.RDP | SUPP_PROTOCOLS.SSL |SUPP_PROTOCOLS.HYBRID
				else:
					self.client_x224_flags = 0
	...
	async def credssp_auth(self):
		try:
			...
			if self.authapi is None:
				...
				from modules.rdp.credssp import CREDSSPCredential
			...
			token = None
			self.authapi.username = self.username
			self.authapi.password = self.password
	...
	async def __send_userdata(self):
		try:
			...
			tzstname = 'Tokyo Standard Time'
			tzdlname = 'Tokyo Daylight Time'

			systz = TS_TIME_ZONE_INFORMATION()
			# systz.Bias = 4294967236
			systz.Bias = 4294966756
			...
			#systz.StandardName = tzstname.encode('utf-16-le')
			...
			#systz.DaylightName = tzdlname.encode('utf-16-le')
			...
			extinfo.clientAddress = ''
			extinfo.clientDir = 'C:\\Windows\\system32\\mstscax.dll'
			extinfo.clientTimeZone = systz
			extinfo.clientSessionId = 4 # 0 1 2 3

			extinfo.cbAutoReconnectCookie = 0
			extinfo.reserved1 = b'\x64\x00'
			extinfo.reserved2 = b'\x00\x00'
			extinfo.cbDynamicDSTTimeZoneKeyName = 38
			extinfo.dynamicDSTTimeZoneKeyName = tzstname.encode('utf-16-le')
			extinfo.dynamicDaylightTimeDisabled = False
			...
			info.flags = INFO_FLAG.ENABLEWINDOWSKEY|INFO_FLAG.MAXIMIZESHELL|INFO_FLAG.UNICODE|INFO_FLAG.DISABLECTRLALTDEL|INFO_FLAG.MOUSE|INFO_FLAG.AUTOLOGON|INFO_FLAG.USING_SAVED_CREDS|INFO_FLAG.LOGONNOTIFY
			info.Domain = ''
	...
	async def __handle_mandatory_capability_exchange(self):
		try:
			...
			# input('target user')
			# print(self.__joined_channels['MCS'].channel_id)
