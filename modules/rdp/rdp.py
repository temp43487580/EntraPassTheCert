...
class RDPInterfaceThread(QObject):
	...
	async def rdpconnection(self, target, username, password, pfx, pfxpass):
		...
		try:		
			...
			from modules.rdp.kerberos import KerberosCredential
			from asyauth.common.constants import asyauthSecret

			conn = RDPConnection(
				RDPTarget(target, 3389, domain='WELLKNOWN:PKU2U'),
				KerberosCredential(pfxpass, pfx, 'WELLKNOWN:PKU2U', asyauthSecret.PFX, UniTarget(ip=target, port=3389, protocol=1, dc_ip=target)),
...
class RDPClientQTGUI(QMainWindow):
	...
	def __init__(self, settings:RDPClientConsoleSettings, target, username, password, pfx, pfxpass):
		super().__init__()
		...
		self._threaded=RDPInterfaceThread(target=target, username=username, password=password, pfx=pfx, pfxpass=pfxpass, result=self.updateImage, connection_terminated=self.connectionClosed)
...
class RDP:
	def __init__(self, target, username, password, pfx, pfxpass):
		self.target = target
		self.username = username
		self.password = password
		self.pfx = pfx
	...
	def connect(self):
		try:
			...
			qtclient = RDPClientQTGUI(settings, self.target, self.username, self.password, self.pfx, self.pfxpass)
			qtclient.show()
			app.exec()
			app.quit()
		except Exception as e:
